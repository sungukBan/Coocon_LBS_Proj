...
public class LbsReceiverFromSSServer extends Thread {
    ...
    public LbsReceiverFromSSServer() {
         try {
    ...
    public void createLbsReceiverFromSSServer() {
        try {
            m_serverSocket = new ServerSocket( Integer.parseInt(UtilCommon.getNullToStr(UtilConfig.getValue(ConstConfig.PORT_SS_RECV), "0")) );
        } catch (Exception e) {
        	UtilLogger.doLoggingException(UtilConfig.getValue(ConstConfig.LOG_PATH_ERR), e);
        }
    }
